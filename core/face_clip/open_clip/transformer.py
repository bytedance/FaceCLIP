...
class VisionTransformer(nn.Module):
    ...
    def get_patch_feature(self, x):
        # assert (x.shape[0] == 1)
        im_size = [(x_.shape[2], x_.shape[3]) for x_ in x]
        x = [self.conv1(x_) for x_ in x] # shape = [*, width, grid, grid]
        x = [x_.reshape(x_.shape[0], x_.shape[1], -1) for x_ in x]  # shape = [*, width, grid ** 2]
        x = [x_.permute(0, 2, 1) for x_ in x]  # shape = [*, grid ** 2, width]

        # class embeddings and positional embeddings
        x = [torch.cat([_expand_token(self.class_embedding, x_.shape[0]).to(x_.dtype), x_], dim=1) for x_ in x]
        # shape = [*, grid ** 2 + 1, width]
        
        x = [x_ + self.interpolate_pos_encoding(x_, sz[0], sz[1]) for x_, sz in zip(x, im_size)]
        max_len = max([x_.shape[1] for x_ in x])
        x = [F.pad(x_, (0, 0, 0, max_len - x_.shape[1])) for x_ in x]
        x = torch.cat(x, dim=0)
    ...
    def patch_padding(self, x):
        ...
        for x_ in x:
            l = x_.shape[1]
            x_0 = F.pad(x_, (0, 0, 0, self.positional_embedding.shape[0] - l), "constant", 0)
            out_feat.append(x_0)
            mask = torch.zeros([1, self.positional_embedding.shape[0]], dtype=dtype, device=device)
            mask[:, :x_.shape[1]] = 1
            out_mask.append(mask)
        out_feat = torch.cat(out_feat, dim=0)
        out_mask = torch.cat(out_mask, dim=0)
        out_mask = (out_mask.unsqueeze(-1) * out_mask.unsqueeze(1)).to(torch.bool) | \
            torch.eye(self.positional_embedding.shape[0], device=device, dtype=torch.bool).repeat([batch_size, 1, 1])
    ...
    def forward(self, x: torch.Tensor):
        x = [x_.unsqueeze(0) for x_ in x]
        x = self.get_patch_feature(x)
        # x, attn_mask = self.patch_padding(x)
